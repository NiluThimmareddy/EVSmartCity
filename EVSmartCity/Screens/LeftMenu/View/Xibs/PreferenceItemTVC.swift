...
class PreferenceItemTVC: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        preferenceTypeButton.setTitleColor(.darkGray, for: .normal)
        preferenceTypeButton.titleLabel?.font = UIFont.systemFont(ofSize: 10, weight: .medium)
        preferenceTypeButton.isUserInteractionEnabled = false
        preferenceTypeButton.contentHorizontalAlignment = .right
        preferenceTypeButton.contentVerticalAlignment = .center
        preferenceTypeButton.titleLabel?.numberOfLines = 1
        preferenceTypeButton.titleLabel?.adjustsFontSizeToFitWidth = false
        preferenceTypeButton.titleLabel?.lineBreakMode = .byTruncatingTail
    }
    ...
    func configure(image: String, title: String, description: String, isSelected: Bool = false) {
        ...
        preferenceTypeButton.setAttributedTitle(attributedString, for: .normal)
        
        preferenceTypeButton.setTitle(description, for: .normal)
        preferenceTypeButton.titleLabel?.font = UIFont.systemFont(ofSize: 10, weight: .medium)
        preferenceTypeButton.contentHorizontalAlignment = .right
        ...
        if isSelected {
            ...
            preferenceTypeButton.setAttributedTitle(selectedAttributedString, for: .normal)
            preferenceTypeButton.setTitleColor(.darkGray, for: .normal)
        } else {
            backView.backgroundColor = .clear
            lineView.isHidden = true
